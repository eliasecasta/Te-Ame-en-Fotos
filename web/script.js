...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const playlist = [
    ...
    {
      file: 'Despacito - Luis Fonsi & Daddy Yankee.mp3',
      title: 'Despacito',
    ...
    () => {
      if (window.scrollY < window.innerHeight) {
        ...
        heroContent.style.opacity =
          1 - (window.scrollY / window.innerHeight) * 1.5;
      }
    },
    { passive: true },
  );
  ...
  images.forEach((img) => {
    ...
    img.style.transition =
      'opacity 1s var(--ease-out), filter 0.8s var(--ease-out), transform 0.8s var(--ease-out)';
  ...
  function installMusicUnlockListeners() {
    if (musicUnlockListenersInstalled || isPlaying || playlist.length === 0)
      return;
    ...
    document.addEventListener('touchstart', unlockMusicFromInteraction, {
      passive: true,
    });
    document.addEventListener('keydown', unlockMusicFromInteraction);
    window.addEventListener('wheel', unlockMusicFromInteraction, {
      passive: true,
    });
    window.addEventListener('scroll', unlockMusicFromInteraction, {
      passive: true,
    });
  }
  ...
  musicPlayBtn.addEventListener('click', () => {
    if (isPlaying) pause();
    else play().catch(installMusicUnlockListeners);
  });
  ...
  if (featureVideo) {
    ...
    const videoSrc =
      'assets/vid/me da mucha risa que en este video sin vernos, sabiamos exactamente las respuestas correctas y nos conociamos tan bien.mov';
    ...
    if (videoUnmuteBtn) {
      videoUnmuteBtn.addEventListener('click', () => {
        if (featureVideo.muted) {
          ...
          videoUnmuteBtn.querySelector('.unmute-text').textContent =
            'Sonido activado';
